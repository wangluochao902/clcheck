...
def test_check_apt_get(translator_clchecker):
    ...
    clchecker.check(command_name, commandline)
    commandline = "apt-get --assume-no install -qqo"
...
def test_tar_CLSyntaxError(translator_clchecker):
    ...
    #     commandline = "tar --create --verbose --file= /dev/rmt0"
    #     clchecker.check(command_name, commandline)
    # assert "Expected 'PATH' or STRING at the position of the star(*) in => 'se --file=* /dev/rmt0'." in e_info.value.message
    ...
    with pytest.raises(errors.CLSyntaxError) as e_info:
        commandline = "tar -cvf"
        clchecker.check(command_name, commandline)
    assert "Expected '=' or '[ \t]+' at the position of the star(*) in => 'tar -cvf*'." in e_info.value.message
